...
describe('LoginPage', () => {
  ...
  it('updates the email input when the user types', async () => {
    ...
    // Act
    await user.type(screen.getByPlaceholderText('Email or Username'), 'demo@example.com');

    // Assert
    expect(screen.getByPlaceholderText('Email or Username')).toHaveValue('demo@example.com');
  });
  ...
  it('calls signIn and onLogin after a successful login', async () => {
    ...
    // Assert
    expect(signIn).toHaveBeenCalledWith({ username: 'demo@example.com', password: 'secret123' });
