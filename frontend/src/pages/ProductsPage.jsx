  ...
  const filteredProducts = useMemo(() => {
    return products
      .filter((product) => {
        ...
        if (category === "Jewelry") {
          matchesCategory = product.category !== "Gemstones";
        } else if (category === "Gemstones") {
          matchesCategory = product.category === "Gemstones";
        } else if (category === "Collections") {
          matchesCategory = product.badge != null;
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl text-stone-800">
              Browse our signature jewelry pieces.
            </h1>
          </div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[250px_1fr]">
        <aside className="rounded-[1.5rem] border border-stone-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 space-y-4 text-sm text-stone-600">
            <div>
              <p className="font-semibold text-stone-800">Metal</p>
              <p className="mt-2">Gold, Rose Gold, Silver, Platinum</p>
            </div>
            <div>
              <p className="font-semibold text-stone-800">Stone</p>
              <p className="mt-2">Diamond, Emerald, Sapphire, Pearl</p>
            </div>
            <div>
              <p className="font-semibold text-stone-800">Price</p>
              <p className="mt-2">₹1,200 - ₹15,000</p>
            </div>
            <div>
              <p className="font-semibold text-stone-800">Availability</p>
              <p className="mt-2">In stock and preorder</p>
            </div>
          </div>
        </aside>
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {filteredProducts.slice(0, 24).map((product) => (
            <div
              key={product.id}
              className="group overflow-hidden rounded-[1.5rem] border border-stone-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg"
            >
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-4 top-4 rounded-full bg-white/90 p-2 text-stone-700"
                >
                  <Heart size={16} />
                </button>
              </div>
